...
# GET /api/v1/books/title/<title_slug> - returns a book by name


@app.route('/api/v1/books/title/<title_slug>', methods=['GET'])
def get_books_by_title(title_slug):
    return jsonify(get_books_by_title_name(title_slug))
...
def get_books_by_title_name(title_slug):
    conn = sqlite3.connect('db.sqlite')
    cursor = conn.cursor()

    # Searching for books where the title contains the slug (LIKE with %)
    query = "SELECT * FROM book WHERE lower(title) LIKE ?;"
    like_pattern = f'%{title_slug.lower()}%'
